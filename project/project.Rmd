...
ggplot(data = year_sex, mapping=aes(x = Year, y =n, fill = Sex)) + 
  geom_bar(stat = "identity", position = "dodge")  +  
  labs(title = "Proportions of Gender for Number of Suicides/100K Population By Year", 
       x = "Year", y = "Number of Suicides/100K Population")
```

Becaus there are interesting trends over time beginning to make themselves evident in the visualization, we made a line graph. 
...
```{r all}
# lm_suicides <- lm(country ~ Year + Sex + age + Population + suicides_no + cls_perc_eval + cls_students + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
# glance(m_score)$adj.r.squared
